...
- 54/minute (а не 1 пакет в 1 сек)
  - В iptables модуль hashlimit не поддерживает миллисекунды. 54/минута = 0.9 пакетов в 1 сек на соединение. Запас в 100 мс нужен, чтобы исключить погрешность возникающую при мгновенном Reject, которая приводит к блокировке подключения с вашего устройства к серверу с mtproto на 2 минуты
- REJECT вместо DROP
