...
_MEKO использует на данный момент [более точную V3 и тестовую V4 версию](https://github.com/Mekotofeuka/MTPROTO_FIX_By_MEKO/blob/main/data/dictionary.md#Термины-связанные-с-фиксом)_
</div>
...
2. **Установить стандартный Telemt**, либо "**MTPROTO.zig**" или **MTG**
   ...
   > (v4 находится на активном тестировании, Если у вас он не работает - используйте v3 вариант. Также если у вас на роутере стоит nfqws2, v4 фикс аналогично может не работать.)
4. Применить наш фикс к прокси нажав **[1] Установить SYN FIX** в главном меню
5. **Отключить встроенные MSS и SYN** из конфига телемт (если он уже был добавлен в конфиг телемт на сервер ранее)
...
# Как работает последняя версия фикса (V4):
...
- **1 слой** - Проверяет является ли устройство ios или нет. 
  ...
  - **Если нет** - Переходим на второй слой и применяем к нему правила второго слоя для всех устройств, которое взаимодействует с пакетом, отправляя его содержимое в ином виде.
...
- Определяем тип устройства - iOS/desktop/android по TCP-отпечатку
 - Если **iOS** — пропускаем пакет без изменений.
 - Для всех остальных устройств включаем обработку:
   - Режем первый пакет на 3 части. Средняя отправляется с битой контрольной суммой.
   - Клиент переотправляет битую часть, и соединение достраивается штатно.
   - V4 не урезает размер пакетов, поэтому медиа грузятся с полной скоростью.
 - В MSS просто нет необходимости для данного билда, поэтому в скрипте добавлена функция его отключения. Если вы оставите у себя старое правило, либо настройку в конфиге с MSS или другим вариантом SYN ограничения, то медиа и скорость загрузки могут быть низкими, так что их рекомендуется закомментировать/удалить с сервера до применения фикса. 
...
- https://github.com/Liafanx/MTproxy-reanimation - схожий по функционалу инструмент(автор занимался разработкой фикса совместно с комьюнити и внёс **большой** вклад)
- https://assyoucandy.github.io/telemt-server-guide/telemt-keepalive-guide.html - старый гайд по оптимизации от комьюнити
- https://h1de0x.github.io/telemt-tune/ - старый гайд по оптимизации от комьюнити
