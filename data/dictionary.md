...
1. **Для SelfSteal-маскировки** используйте домены, которые уже поддерживают X25519MLKEM768. Это можно проверить через бота `@Sni_checker_bot` или встроенный чекер в проекте.

2. **Если используете nginx** для SelfSteal — убедитесь, что на сервере стоит OpenSSL 3.5+ и nginx собран с его поддержкой.

3. **Альтернатива nginx** — используйте Caddy, он работает с гибридным шифрованием из коробки.

4. **Если прокси не работает на iOS** — скорее всего, домен не поддерживает постквантовый алгоритм, и сервер отвечает через X25519. Проверьте домен через чекер.
